...
begin
 ...
 FHSLBmp.Width:= MaxHue;//AT//240;
 FHSLBmp.Height:= 241;
 Width := Pred(MaxHue);//AT//239;
 Height := 240;
...
begin
 ...
 for Hue := 0 to Pred(MaxHue) do//AT//239 do
  for Sat := 0 to 240 do
   begin
...
begin
 ...
 FSelected := c;
 FManual := false;
 mxx := Round(FHue*(Width/Pred(MaxHue)));
...
begin
 if h > Pred(MaxHue) then h := Pred(MaxHue);//AT
 if h < 0 then h := 0;
 FHue := h;
