...
rem SynWrite.zip
start /min /wait winrar a %a% *.wlx *.inf Syn.exe SynHelper.exe Settings\Enc.cfg HL\* Readme\* Lang\* Data\lexlib Data\autocomplete\* Data\colors\* Data\newdoc\* Data\outpresets\* Data\skins\* Data\conv\* Data\icons\* Data\htmltidy\* Data\websearch\* Tools\* Plugins\SynFTP\* *.dll python3*.* *.manifest DLLs\* Py\sw*.py 
...
start /min /wait winrar a -r -x*.pyc %a% Py\syn_plugin_manager Py\syn_color_picker Py\syn_html_tidy Py\syn_make_plugin Py\syn_insert_time Py\syn_comments Py\syn_new_file
